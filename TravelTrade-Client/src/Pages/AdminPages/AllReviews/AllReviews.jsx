  ...
  const fetchAllSenderReviews = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:9000/users/reviews/all-given-by-senders`
      );
      if (response.data) {
  ...
  const applyFilters = () => {
    ...
    if (filterRole !== "all") {
      if (filterRole === "sender-reviews")
        filtered = filtered.filter((r) => r.reviewerRole === "sender");
      else if (filterRole === "traveler-reviews")
        filtered = filtered.filter((r) => r.reviewerRole === "traveler");
      else if (filterRole === "reviews-for-travelers")
        filtered = filtered.filter((r) => r.reviewedUserRole === "traveler");
      else if (filterRole === "reviews-for-senders")
        filtered = filtered.filter((r) => r.reviewedUserRole === "sender");
    }
    if (searchTerm) {
        ...
        (review) =>
          ...
          (review.reviewedUserName &&
            review.reviewedUserName
              ...
              .includes(searchTerm.toLowerCase())) ||
          (review.parcel_type &&
            review.parcel_type.toLowerCase().includes(searchTerm.toLowerCase()))
      );
    }
    filtered.sort((a, b) => {
      if (sortBy === "newest")
  ...
  const getStatistics = () => {
    ...
    const averageRating =
      totalReviews > 0
        ...
        : 0;
    const senderReviews = filteredReviews.filter(
      (r) => r.reviewerRole === "sender"
    ).length;
    const travelerReviews = filteredReviews.filter(
      (r) => r.reviewerRole === "traveler"
    ).length;
    const ratingDistribution = { 5: 0, 4: 0, 3: 0, 2: 0, 1: 0 };
    filteredReviews.forEach((r) => ratingDistribution[r.rating]++);
    return {
      totalReviews,
      averageRating,
      senderReviews,
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="bg-gradient-to-r from-[#009ee2] to-[#0085c3] rounded-xl shadow-xl px-8 py-6 mb-8 text-white">
          ...
          <p className="text-blue-100">
            Monitor and manage all reviews given by senders
          </p>
        </div>
        {/* Statistics Overview */}
        ...
        <div className="bg-white rounded-xl shadow border p-6">
          <div className="flex flex-wrap items-center gap-4 mb-4">
            ...
            >
              ...
              <option value="reviews-for-travelers">
